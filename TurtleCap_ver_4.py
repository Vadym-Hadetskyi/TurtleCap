...
class App:
    ...
    def onChange(self, value):
        self.vid.vid.set(cv2.CAP_PROP_POS_FRAMES, value)
        self.vid.Q.clear()
        time.sleep(0.5)
    ...
    def flash_forward(self):
        ...
        self.delay = 100
        plus10sec = min(round(self.vid.vid.get(cv2.CAP_PROP_POS_FRAMES) + 20.0*self.vid.vid.get(cv2.CAP_PROP_FPS)),
                        self.vid.length)
        self.vid.vid.set(cv2.CAP_PROP_POS_FRAMES, plus10sec)
    ...
    def flash_backward(self):

        self.vid.OnHold = True
        self.flash_backward_button.configure(state='disabled')
        self.delay = 100
        minus10sec = max(1.0,
                         round(self.vid.vid.get(cv2.CAP_PROP_POS_FRAMES) - 20.0*self.vid.vid.get(cv2.CAP_PROP_FPS)))
        self.vid.vid.set(cv2.CAP_PROP_POS_FRAMES, minus10sec)
        for i in range(int(self.vid.Q.qsize())):
            self.vid.read()
        self.vid.OnHold = False
        self.delay = 1
    ...
    def open_video(self):
        ...
        try:
            ...
            # trackbar
            # self.trackbar = cv2.createTrackbar('progress', 'Twindow', 0, self.vid.length, self.onChange)
        except:
...
# csv = /Users/thewaveorthemountain/Formation/S2/Project/Turtles/behaviors_test.csv
